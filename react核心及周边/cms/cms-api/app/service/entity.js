...
module.exports = class extends BaseService {
  ...
  async create(body) {
    ...
    try {
      ...
      const controllerTemplate = await fs.readFile(
        path.join(__dirname, "../view/controller.js"),
        "utf8"
      );
      const controllerContent = await this.ctx.renderString(controllerTemplate, {
        name: body.name,
      });
      await fs.writeFile(
        path.join(__dirname, `../controller/${body.name}.js`),
        controllerContent,
        "utf8"
      );
      ...
      await fs.writeFile(
        path.join(__dirname, `../service/${body.name}.js`),
        serviceContent,
        "utf8"
      );
  ...
  async update(entity) {
    ...
    const newFields = fields.filter(
      (field) => !columns.find((column) => column.COLUMN_NAME === field.name)
    );
        ...
        node.body = [
          ...
          types.expressionStatement(
            types.callExpression(
              types.memberExpression(types.identifier("router"), types.identifier("resources")),
              [
                types.stringLiteral(name),
                types.stringLiteral(`/${name}`),
                types.memberExpression(types.identifier("controller"), types.identifier(name)),
              ]
            )
          ),
        ];
